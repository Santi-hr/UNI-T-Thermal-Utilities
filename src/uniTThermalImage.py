...
class UniTThermalImage:
    ...
    def init_from_image(self, filepath):
        ...
        self._get_file_time_os(path_in)

        # Read BMP header
        self._get_bmp_header()
        # Get grayscale image after .bmp
        byte_offset = self._extract_grayscale_img()

        # Get colorbar after grayscale image
        byte_offset = self._extract_colorbar(byte_offset)

        # Get embedded temperature after colorbar
        byte_offset = self._extract_temp_data(byte_offset)

        # Get embedded timestamp after embedded data
        self._extract_file_time(byte_offset)
    ...
    def export_bmp(self):
        ...
        self._serialize_additional_data(output_bytes)
    ...
    def _get_file_time_os(self, path_in):
    ...
    def _get_bmp_header(self):
        ...
        self.bmp_header['file_size'] = self._read_int32(2)
        self.bmp_header['reserved_1'] = self._read_int16(6)
        self.bmp_header['reserved_2'] = self._read_int16(8)
        self.bmp_header['data_start_byte'] = self._read_int32(10)
        self.bmp_header['bmp_header_size'] = self._read_int32(14)
        self.bmp_header['img_width_px'] = self._read_int32(18)
        self.bmp_header['img_height_px'] = self._read_int32(22)
        self.bmp_header['plane_num'] = self._read_int16(26)
        self.bmp_header['bits_per_px'] = self._read_int16(28)
        self.bmp_header['compression'] = self._read_int32(30)
        self.bmp_header['img_size'] = self._read_int32(34)
        self.bmp_header['horizontal_res'] = self._read_int32(38)
        self.bmp_header['vertical_res'] = self._read_int32(42)
        self.bmp_header['color_table_size'] = self._read_int32(46)
        self.bmp_header['important_colors'] = self._read_int32(50)
    ...
    def _extract_temp_data(self, byte_index_input):
        ...
        self.temp_max = self._read_int16(byte_index_input + 1) / 10.0
        self.temp_min = self._read_int16(byte_index_input + 3) / 10.0
        # Byte 5, unknown. Always 255
        # Byte 6, unknown. Always 0
        self.temp_center = self._read_int16(byte_index_input + 7) / 10.0
        self.emissivity = self.file_bytes[byte_index_input + 9] / 100.0
        # Byte 10, unknown. Always 6
        # Byte 11, 12 & 13, unknown. Always 0
        self.temp_min_pos_w = self._read_int16(byte_index_input + 14)
        self.temp_min_pos_h = self._read_int16(byte_index_input + 16)
        self.temp_max_pos_w = self._read_int16(byte_index_input + 18)
        self.temp_max_pos_h = self._read_int16(byte_index_input + 20)
        self.temp_center_pos_w = self._read_int16(byte_index_input + 22)
        self.temp_center_pos_h = self._read_int16(byte_index_input + 24)

        byte_offset = byte_index_input + 26
        return byte_offset

    def _extract_file_time(self, byte_index_input):
        ...
        if byte_index_input != len(self.file_bytes):
            self.img_datetime = datetime.fromtimestamp(self._read_int32(byte_index_input))

    def _serialize_additional_data(self, output_bytes):
        ...
        """
        self._serialize_file_time(output_bytes)

    def _serialize_file_time(self, output_bytes):
